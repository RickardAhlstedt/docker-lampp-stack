...
As of now, there are several different PHP versions. Use appropriate php version as needed:
...
- Run `dev/setup.sh` and fill in the info
- Run `dev/start.sh` to start the container
...
> Please select Mariadb as Database. Oracle doesn't build their SQL Containers for the arm Architecure, using e.g mysql8 can trigger a warning regarding wrong architecture.
...
> If you want to install more extension, just update `./bin/webserver/Dockerfile`. You can also generate a PR and I will merge if it seems good for general purpose.
> You have to rebuild the docker image by running `docker-compose build` and restart the docker containers.
...
I would be happy if you want to create a pull request or help people with their issues. If you want to create a PR, please remember that this stack is not built for production usage, and changes should be good for general purpose and not overspecialized.

Please create your PR against master branch.

## Why you shouldn't use this stack unmodified in production

This is meant to be a quick and easy way to setup a local development for any PHP-project, and may not be suitable to production.
